...
func AuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if authHeader == "" {
			...
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"status": false, "statusCode": http.StatusUnauthorized, "message": constants.AUTHORIZATION_HEADER})
			return
		}
		parts := strings.SplitN(authHeader, " ", 2)
		if len(parts) != 2 || !strings.EqualFold(parts[0], "bearer") {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"status": false, "statusCode": http.StatusUnauthorized, "message": constants.BAD_AUTH_FORMAT})
			return
		}
		...
		if err != nil {
			...
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"status": false, "statusCode": http.StatusUnauthorized, "message": constants.SESSION_NOT_FOUND})
			return
		}
...
func GuardMiddleware(guardName string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !exists {
			c.AbortWithStatusJSON(http.StatusForbidden, gin.H{"status": false, "statusCode": http.StatusForbidden, "message": constants.ACCESS_DENIED})
			return
		}
